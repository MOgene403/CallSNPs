...
sub workerThread{
	while(my $work=$q->dequeue_nb()){
		...
		my $parser	= $FindBin::Bin."/Scripts/".$config->get("PATHS","Parser");
...
#			my $baseOutput = $TempDir."/".$grp."_vs_".$alias;
			...
			warn $command."\n";
			`$command`;
			my $csvOut = $config->get("DIRECTORIES","Output")."/".$grp.".csv";
			$command = "perl $parser $baseOutput.raw.vcf $snpRate $minCov $csvOut";
			`$command`;
