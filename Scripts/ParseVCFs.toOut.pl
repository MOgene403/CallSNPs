...
my $usage = "perl $0 <input VCF> <cutoff for including alternative bases (frequency)> <min coverage req> <output CSV>\n\n";
...
#Notch_3_HC      206     .       T       C,X     0       .       DP=6211;I16=2797,2859,271,196,200591,7229181,16142,571640,282413,14105661,20858,933236,101978,2266550,8989,199349;QS=0.925546,0.074454,0.000000,0.000000;VDB=2.007198e-01;RPB=-1.554554e-01     PL      0,255,255,255,255,255
	...
	for(my$i=1;$i<=$#Q;$i++){
		if($Q[$i]>=$rate){
			next unless _checkBiDirectionalSupport($I16[2],$I16[3]);
			$good=1;
			my $this_out = $out;
			$this_out.=",".$A[$i-1].",".$Q[$i];
			push @output, $this_out;
		}else{
		}
	}
...
sub _checkBiDirectionalSupport {
	my @support=sort {$a <=> $b} @_;
	my $rat = $support[0]/$support[1];
	if($rat<.1){
		return -1;
	}else{
		return 1;
